...
def verify_user(nomeCompleto, usuario, funcao):
    """
    Verifica se o usuario já existe dentro do Banco de Dados
    :param nomeCompleto: Nome Completo do Aluno
    :param usuario: Usuário ja pre feito
...
def att_hora(usuario, hrI, hrF):
    """
    Atualiza o tempo de uso de um usuário após ele finalizar a sessão
    :param usuario: usuario do aluno
    :param hrI: hora do login
    :param hrF: hora do logoff
    :return: none
    """
    alunos = {}
    with open('usuarios.json', 'r') as file:
...
def alterar_senha(lst, usuario):
    """
    Função que altera a senha de uma conta
    :param lst: Banco de Dados
    :param usuario: usuario a alterar a senha
    :return: none
    """
    log = lst[usuario]
    password = ''
